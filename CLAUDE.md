# production.atj — project context

Consolidated context from every Claude Code chat session for this project, read start to finish. Much of it is not derivable from the code — it records decisions, rejected alternatives, and traps that already cost time once. **Read it before starting work, and update the relevant section when a session makes a decision, establishes a convention, or resolves/adds an open item.**
...
- **Context generated:** 2026-08-11 · from 11 project sessions (7 with full transcripts, 4 cleaned up and listed by title only)
