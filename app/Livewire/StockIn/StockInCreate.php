...
class StockInCreate extends Component
{
    ...
    public function mount(ApiService $api,$id = null, $viewStatus = null)
    {
        ...
        if ($id) {
            ...
            $this->stockIn = StockIn::with('reportRawMaterials')->findOrFail($id);
            $this->warehouses = $api->get('/v1/warehouses', ['module' => 'production'])['data'] ?? [];
    ...
    public function submit()
    {
        ...
        try {
            ...
            foreach ($this->rawMaterials as $item) {
                $inventory = WarehouseInventory::where('warehouse_id', $this->warehouse_id)
            ...
            DB::commit();
            return to_route('stock-ins')->with('success', $this->editing ? 'Stock In updated successfully!' : 'Stock In created successfully!');
