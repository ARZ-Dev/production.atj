...
class TransferCreate extends Component
{
    ...
    public function mount(ApiService $api, $id = null, $viewStatus = null)
    {
        ...
        if ($id) {
            ...
            $this->transfer = Transfer::with('reportRawMaterials')->findOrFail($id);
        ...
        try {
            if ($this->editing) {
                if ($this->transfer->status !== 'pending') {
                    DB::rollBack();
                    return to_route('transfers')->with('error', 'Cannot edit transfer that has already been processed!');
                }
            ...
            DB::commit();
            return to_route('transfers')->with('success', $this->editing ? 'Transfer updated successfully!' : 'Transfer created successfully!');
    ...
    public function confirmLoad()
    {
        ...
        try {
            ...
            DB::commit();
            return to_route('transfers')->with('success', 'Transfer load approved successfully!');
    ...
    public function confirmReceive()
    {
        ...
        try {
            ...
            DB::commit();
            return to_route('transfers')->with('success', 'Transfer receive approved successfully!');
