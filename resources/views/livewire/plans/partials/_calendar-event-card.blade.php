...
<div class="pbc-card" draggable="true" data-event-id="{{ $event->id }}" wire:key="pbc-event-{{ $event->id }}"
    style="left: calc(var(--pbc-hour-w) * {{ $fromHour }}); width: calc(var(--pbc-hour-w) * {{ $spanHours }} - 4px); top: {{ $track * 36 + 4 }}px;"
    ...
    <span class="pbc-card-type">{{ $event->eventType->name ?? 'No type' }}</span>
    <span class="pbc-card-time">{{ \Carbon\Carbon::parse($event->from_time)->format('H:i') }}</span>
    @if($hasRecipe && !$event->calculated_duration)
        <i class="bi bi-exclamation-circle text-warning ms-1" title="Assign a line/preparation in the lanes view to calculate duration"></i>
    @endif
</div>
